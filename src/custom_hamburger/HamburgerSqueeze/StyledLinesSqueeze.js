...
const activeBefore = `
  top: 0;
  opacity: 0;
  transition: top 0.5s ease, opacity 0.5s 0.12s ease;
`

const activeAfter = `
  bottom: 0;
  transform: rotate(-90deg);
  transition: bottom 0.5s ease, transform 0.5s 0.12s cubic-bezier(0.215, 0.61, 0.355, 1);
`

export const StyledLinesSqueeze = styled(StyledLines)`
  &::before {
    transition: top 0.5s 0.12s ease, opacity 0.5s ease;
    ${({ isActive }) => isActive && activeBefore};
    ${getBarColor};
  }
  &::after {
    transition: bottom 0.5s 0.12s ease,
